---
layout: page
title: press & media
permalink: /press-media/
description: Press features, invited talks, and public-facing resources
nav: false
---

## Press & Media

This page highlights interviews, media coverage, and public-facing resources connected to lab research in macroecology, biodiversity informatics, and global change biology.

---

## Public Resources

- [Lab website archive](https://brianjenquist.wordpress.com/)
- [Google Scholar](https://scholar.google.com/citations?user=mAbA6EoAAAAJ&hl=en)
- [ResearchGate](https://www.researchgate.net/profile/Brian_Enquist)
- [FigShare lectures and resources](https://figshare.com/authors/Brian_Enquist/663712)

---

## Selected Public Projects

- SPARC biodiversity planning resources and applications
- Trait-based ecology and global change outreach talks
- Open biodiversity data and reproducible science materials

For media requests, please use the [contact page]({{ '/contact/' | relative_url }}).
