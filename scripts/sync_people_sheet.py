...
def main():
    ...
    # -----------------------------------------------------------------------
    # Load and merge manual alumni overrides (preserved across sheet syncs)
    # -----------------------------------------------------------------------
    # Manual overrides (_data/alumni_overrides.yml) allow curated alumni entries
    # to persist even if they are removed from or missing in the Google Sheet.
    # This prevents accidental data loss during sync automation.
    ALUMNI_OVERRIDES_FILE = REPO_ROOT / "_data" / "alumni_overrides.yml"
    manual_alumni_by_name = {}
    if ALUMNI_OVERRIDES_FILE.exists():
        try:
            overrides_data = yaml.safe_load(ALUMNI_OVERRIDES_FILE.read_text(encoding="utf-8"))
            if isinstance(overrides_data, dict) and "alumni" in overrides_data:
                for entry in overrides_data["alumni"]:
                    if isinstance(entry, dict) and "name" in entry:
                        manual_alumni_by_name[entry["name"]] = entry
                if manual_alumni_by_name:
                    print(f"\nLoaded {len(manual_alumni_by_name)} manual alumni overrides from {ALUMNI_OVERRIDES_FILE.relative_to(REPO_ROOT)}")
        except Exception as e:
            print(f"WARNING: Could not load alumni overrides ({e})", file=sys.stderr)

    # Merge: ensure all manual entries are present in final alumni list
    # (by name, so sheet updates take precedence for shared names)
    ...
    FILE_HEADER = (
        ...
        "# Alumni overrides preserved from _data/alumni_overrides.yml\n"
        "# ============================================================\n\n"
    )
